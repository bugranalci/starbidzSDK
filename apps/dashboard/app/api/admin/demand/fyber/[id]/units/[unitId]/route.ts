...
// Fyber Spot ID validation: numeric string
const fyberSpotIdRegex = /^[0-9]+$/

const updateAdUnitSchema = z.object({
  name: z.string().min(1).max(100).optional(),
  externalId: z.string().min(1).max(255).regex(fyberSpotIdRegex, {
    message: 'Invalid Fyber Spot ID format. Use numeric characters only.',
  }).optional(),
...
export async function GET(
  ...
  try {
    ...
    console.error('Get Fyber ad unit error:', error)
...
export async function PUT(
  ...
  try {
    ...
    console.error('Update Fyber ad unit error:', error)
    return NextResponse.json({ error: 'Internal error' }, { status: 500 })
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Delete Fyber ad unit error:', error)
    return NextResponse.json({ error: 'Internal error' }, { status: 500 })
  }
}
