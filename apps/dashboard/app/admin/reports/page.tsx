...
const publisherStats = [
  { name: "GameStudio Inc", apps: 5, impressions: 4234567, revenue: 35230.50, netRevenue: 28184.40, status: "active" },
  { name: "AppMakers Ltd", apps: 3, impressions: 3456789, revenue: 28450.30, netRevenue: 22760.24, status: "active" },
  { name: "MobileGames Co", apps: 8, impressions: 2345678, revenue: 19870.40, netRevenue: 15896.32, status: "active" },
  { name: "CasualPlay", apps: 2, impressions: 1897533, revenue: 15879.30, netRevenue: 12703.44, status: "active" },
  { name: "IndieDev Studio", apps: 1, impressions: 1300000, revenue: 10000.00, netRevenue: 8000.00, status: "inactive" },
]

const adUnitStats = [
  { name: "Banner - Home", format: "BANNER", bidRequests: 250000, impressions: 200000, fillRate: 80, ecpm: 3.5, grossRevenue: 700, netRevenue: 560 },
  { name: "Interstitial - Level Complete", format: "INTERSTITIAL", bidRequests: 150000, impressions: 120000, fillRate: 80, ecpm: 12, grossRevenue: 1440, netRevenue: 1152 },
  { name: "Rewarded - Extra Lives", format: "REWARDED", bidRequests: 100000, impressions: 85000, fillRate: 85, ecpm: 18, grossRevenue: 1530, netRevenue: 1224 },
]
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Filter reports by publisher, app, and date range</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="space-y-2">
              <Label>App</Label>
              <Select value={selectedApp} onValueChange={setSelectedApp} disabled={!selectedPublisher}>
                <SelectTrigger>
          ...
          {selectedPublisher && selectedPublisherData && (
            <div className="mt-4 p-3 bg-violet-50 dark:bg-violet-950/20 rounded-lg border border-violet-200 dark:border-violet-800">
              <span className="text-violet-700 dark:text-violet-300 text-sm">
      ...
      <div className="grid gap-4 md:grid-cols-6">
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Bid Requests</CardDescription>
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-2xl text-green-600">${(overviewStats.totalRevenue * 0.8).toLocaleString()}</CardTitle>
          </CardHeader>
        </Card>
      </div>
                ...
                <TableBody>
                  {publisherStats.map((pub) => (
                    <TableRow key={pub.name}>
                      ...
                      <TableCell className="text-right text-green-600">${pub.netRevenue.toLocaleString()}</TableCell>
                      <TableCell>
        ...
        <TabsContent value="adunits" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Detailed metrics by ad unit</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {adUnitStats.map((unit) => (
                    <TableRow key={unit.name}>
                      <TableCell className="font-medium">{unit.name}</TableCell>
                      <TableCell>
                        <Badge variant="outline">{unit.format}</Badge>
                      </TableCell>
                      <TableCell className="text-right">{unit.bidRequests.toLocaleString()}</TableCell>
                      <TableCell className="text-right">{unit.impressions.toLocaleString()}</TableCell>
                      <TableCell className="text-right">{unit.fillRate}%</TableCell>
                      <TableCell className="text-right">${unit.ecpm.toFixed(2)}</TableCell>
                      <TableCell className="text-right">${unit.grossRevenue.toLocaleString()}</TableCell>
                      <TableCell className="text-right text-green-600">${unit.netRevenue.toLocaleString()}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
